...
public interface UserChapterProgressDAO extends JpaRepository<UserChapterProgress,Long> {
    @Query("SELECT ucp FROM UserChapterProgress ucp WHERE ucp.chapter = :chapter AND ucp.user = :user")
    List<UserChapterProgress> findByChapterAndUser(Chapter chapter, User user);
